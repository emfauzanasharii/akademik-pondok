...
class Absensi extends CI_Controller{
	...
	function index(){
		...
		$x['data']=$this->m_siswa->get_siswa_by_id($id);
		$this->load->view('admin/v_absensi',$x);
	}else{
		redirect('login');
	}
	...
	function lihat(){
		...
		$sampaitanggal=$this->input->post('sdate');
		$x['data']=$this->m_absensi->lihat_absen($tanggal,$idadmin);
	...
	function tambah_absen(){
		$id = $this->session->userdata('idadmin');
		$siswa=$this->m_siswa->get_all_siswa_guru($id);
		$no=0;
        foreach ($siswa->result_array() as $i) :
        $no++;
    	$status=strip_tags($this->input->post('status'.$no));
    	$data = [
    		'absen_siswa_id' => $i['siswa_id'],
    		'absen_guru_id' => $this->session->userdata('idadmin'),
    		'absen_status' =>$status,
    		'absen_tanggal'=>$this->input->post('date'),
    	];
