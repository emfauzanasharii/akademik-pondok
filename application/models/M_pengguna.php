...
class M_pengguna extends CI_Model{
	...
	function get_all_pengguna($id){
		$hsl=$this->db->query("SELECT tbl_pengguna.* FROM tbl_pengguna where pengguna_id != '$id'");
		return $hsl;	
	}
	function get_all_pengguna_wali(){
	...
	function simpan_pengguna($nama,$jenkel,$username,$pass_lihat,$password,$email,$nohp,$level,$gambar){
		$hsl=$this->db->query("INSERT INTO tbl_pengguna (pengguna_nama,pengguna_jenkel,pengguna_username,pengguna_pass_lihat,pengguna_password,pengguna_email,pengguna_nohp,pengguna_level,pengguna_photo) VALUES ('$nama','$jenkel','$username','$pass_lihat',md5('$password'),'$email','$nohp','$level','$gambar')");
		return $hsl;
	}
